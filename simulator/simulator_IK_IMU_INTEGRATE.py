# sim_world_tilting_base_level_top_noFK.py
import numpy as np
...
from IK import compute_alpha_rad  # we will *not* use FK at all here
...
def jacobian_alpha_wrt_rpy(R_hat, eps_rot=1e-3):
    ...
    """
    # nominal angles at current pose
...
def run():
    # --- Sim/servo params ---
    DT = 0.02                       # s, UI tick
    ALPHA_RATE_MAX_DEG = 360.0      # deg/s, command slew cap (~servo speed)
    TAU_SERVO = 0.04                # s, internal servo following lag (sim)
    DAMP_JAC = 1e-4                 # damped least squares for J^+ (stability)
    ...
    # --- Pose estimate (top wrt base), initialized level at z0 ---
    R_hat = np.eye(3)               # our running estimate (replaces FK)
    ...
    # --- Drawing (we’ll transform to world each update) ---
    ...
    def update(_):
        ...
        # 2) Controller goal: keep TOP LEVEL IN WORLD.
        # Desired top in BASE frame: R_tb_des = R_bw^T
        R_tb_des = R_bw.T
        T_tb_des = np.array([0.0, 0.0, z0])

        # 3) IK at desired pose → desired servo angles
        ...
        if not ik_ok:
            ...
            delta_cmd = (delta_cmd + np.pi) % (2*np.pi) - np.pi  # shortest turn
        ...
        # 6) POSE ESTIMATION *without FK*: integrate joint changes using IK Jacobian
        #    Get joint change this tick
        delta_alpha = (alpha_meas - alpha_prev)
        alpha_prev = alpha_meas.copy()

        #    Build J_{alpha, rpy} at current pose estimate R_hat
        J, alpha_nom = jacobian_alpha_wrt_rpy(R_hat, eps_rot=1e-3)

        #    Solve J * dtheta ≈ delta_alpha (3x1), damped least squares
        #    dtheta = (J^T J + λ^2 I)^(-1) J^T delta_alpha
        JTJ = J.T @ J + (DAMP_JAC**2) * np.eye(3)
        dtheta = np.linalg.solve(JTJ, J.T @ delta_alpha)

        #    Update pose estimate
        R_hat = R_hat @ so3_exp(dtheta)
        T_hat = np.array([0.0, 0.0, z0])  # keep heave fixed here

        # 7) Draw in WORLD frame (transform base->world)
        #    Compute geometry in BASE from current *estimate* (R_hat)
        ...
        top_frame[:]  = draw_frame(ax3d, R_bw @ R_hat, R_bw @ T_hat, scale=3.0, linewidth=2.0, alpha=1.0)  # top frame (estimated)
        ...
        # IMU readout (top vs world), using our estimate
        R_tw_est = R_bw @ R_hat
        # quick roll/pitch extractor (same as before but inline)
